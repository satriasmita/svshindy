...
use Yii;
use frontend\models\Manual;
use frontend\models\ManualSearch;
...
class ManualController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $manual = Manual::find()->where('manual_id');
        $pagemanual = new Pagination(['totalCount' => $manual->count(), 'pageSize'=> 10]);
        $allManual = $manual->offset($pagemanual->offset)
        ->limit($pagemanual->limit)
        ->all();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            return $this->redirect(['view', 'id' => $model->manual_id]);
        }
