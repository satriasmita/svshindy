...
/**
 * Class m181127_030010_tb_biodata_wni
 */
class m181127_030010_tb_biodata_wni extends Migration
{
    /**
     * {@inheritdoc}
     */
    ...
    if ($this->db->driverName === 'mysql') {
        ...
        $this->createTable('{{%biodata_wni}}',[
        'NIK'=>$this->primaryKey(16),
        'NO_KTP'=>$this->string(40),
        ...
        $this->addForeignKey("fk_biodata_wni_desa", "{{%biodata_wni}}", "NO_KEL", "{{%desa}}", "desa_id", 'CASCADE', 'CASCADE');
        $this->addForeignKey("fk_biodata_wni_kecamatan", "{{%biodata_wni}}", "NO_KEC", "{{%kecamatan}}", "kecamatan_id", 'CASCADE', 'CASCADE');
        $this->addForeignKey("fk_biodata_wni_kota", "{{%biodata_wni}}", "NO_KAB", "{{%kota}}", "kota_id", 'CASCADE', 'CASCADE');
        $this->addForeignKey("fk_biodata_wni_provinsi", "{{%biodata_wni}}", "NO_PROP", "{{%provinsi}}", "provinsi_id", 'CASCADE', 'CASCADE');
        ...
        */
        public function safeDown()
            {
        $this->dropForeignKey('fk_biodata_wni_desa', '{{%biodata_wni}}');
        $this->dropForeignKey('fk_biodata_wni_kecamatan', '{{%biodata_wni}}');
        $this->dropForeignKey('fk_biodata_wni_kota', '{{%biodata_wni}}');
        $this->dropForeignKey('fk_biodata_wni_provinsi', '{{%biodata_wni}}');
        $this->dropTable('{{%biodata_wni}}');
            }
    ...
    {
        echo "m181127_030010_tb_biodata_wni cannot be reverted.\n";
